...
## NETWORK
get_wifi_status() { nmcli -t -f WIFI g 2>/dev/null || echo "disabled"; }
get_wifi_ssid() { local ssid=$(nmcli -t -f ACTIVE,SSID dev wifi 2>/dev/null | grep '^yes' | cut -d: -f2); echo "${ssid:-}"; }
get_wifi_strength() { local signal=$(nmcli -f IN-USE,SIGNAL dev wifi 2>/dev/null | grep '^\*' | awk '{print $2}'); echo "${signal:-0}"; }
get_wifi_icon() {
...
get_bt_icon() {
    if [ "$(get_bt_status)" = "on" ]; then
        if bluetoothctl devices Connected 2>/dev/null | grep -q "Device"; then echo "󰂱"
        else echo "󰂯"; fi
    else echo "󰂲"; fi
}
...
## BATTERY
get_battery_percent() {
    if [ -f /sys/class/power_supply/BAT*/capacity ]; then cat /sys/class/power_supply/BAT*/capacity 2>/dev/null | head -n1
    else echo "100"; fi
}
get_battery_status() {
